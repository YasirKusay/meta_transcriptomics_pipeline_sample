...
def blast_sr(args: argparse.Namespace):
    ...
    start = time.time()
    blast_command = blast_path + " -query " + combined_file_fa + " -db " + args.blast_ncbi_nt_database + " -out " + nt_alignments_file + " -outfmt \"6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen staxids sscinames\" -max_target_seqs 10 -num_threads " + str(args.threads)
