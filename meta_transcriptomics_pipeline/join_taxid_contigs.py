...
def join(accession_file, mapping_file, join_file, path):
    ...
    new_command = subprocess.run("cat " + accession_file + " | cut -f1,2" + " > " + combined_file_temp, shell=True)
    ...
    new_command = subprocess.run("cut -f2 " + combined_file_sorted + " | uniq | egrep -v 'SNAP|Illumina|unsorted|LN:' | LC_COLLATE=C sort -k1 | cut -d'.' -f1 > " + unique_accessions, shell=True)
...
def join_taxid_contigs(output_contig_snap, output_contig_diamond, nucl_map, prot_map, final_out, path):
    relevant_taxids = path + "/relevant_taxids.txt"
    if os.path.isfile(relevant_taxids):
        os.remove(relevant_taxids)
    if join(output_contig_snap, nucl_map, final_out, path) is False or join(output_contig_diamond, prot_map, final_out, path) is False: 
        return False
